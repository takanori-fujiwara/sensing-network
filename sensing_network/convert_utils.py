...
def line_to_box(p0, p1, extrude_dir1, extrude_dir2, width, capping=True):
    ...
    return (
        pv.Line(p0_, p1_)
        .extrude(u1 * width, capping=capping)
        .extrude(u2 * width, capping=capping)
        .triangulate()
    )
...
def output_to_stl(
        ...
        for h_path in h_paths:
            ...
            for i in range(1, len(h_path)):
                ...
                if mag > 0:
                    ...
                    h_path_object = line_to_box(
                        pos1, pos2, u_xy, u_z, resistors_h_paths_width
                    )
    ...
    for v_boxes, (s, t) in zip(resistors_v_boxes, resistor_links):
        for v_box in v_boxes:
            ...
            if np.linalg.norm(pos1 - pos2) > 0:
                ...
                v_path_object = line_to_box(
                    pos1, pos2, u_xy1, u_xy2, resistors_v_boxes_width
                )
    ...
    for in_out_node in [in_node, out_node]:
        if in_out_node is not None:
            ...
            mean_vec = np.array(
                [node_positions_[t] - node_positions_[s] for s, t in related_links]
            ).mean(axis=0)
            if (np.linalg.norm(mean_vec) == 0) or (np.isnan(np.linalg.norm(mean_vec))):
                mean_vec = np.array([1, 0, 0])
            mean_vec /= np.linalg.norm(mean_vec)

            pos1 = node_positions_[in_out_node]
            pos2 = (
                node_positions_[in_out_node]
                - (node_radius + in_out_nodes_cylinder_height) * mean_vec
            )
            in_out_node_object = line_to_cylinder(
                pos1, pos2, in_out_nodes_cylinder_width
            )
...
def output_to_3dforce_json(
    nodes, links, node_positions, node_radius=12.5, link_radius=3.15, outfile_path=None
):
    """
    ...
    for link in links:
        network["links"].append({"source": f"{link[0]}", "target": f"{link[1]}"})
...
def to_lcapy_circuit(
    ...
    cct.add(f"Rarduino {to_cct_node(out_node)} {ground} {arduino_inner_resistance}")
    ...
    if touch_node is not None:
        # capacitor
        cct.add(f"Ctouch {ground} {to_cct_node(touch_node)} {touch_capacitance}")
...
class LcapyCircuitSimplifier:
    ...
    def simplify(
        self, cct, remove_wires=False, array_style=True, no_simplification=False
    ):
            ...
            if c.startswith("R"):
                ...
                if resistance.startswith("{"):
                    ...
                    if expr in self.expr_to_resistance_name:
                        ...
                        resistance_name = (
                            f"{self.simplified_symbol_prefix}{n_combined_resistors}"
                        )
...
def to_ngspice_circuit(
    ...
    # arduino inner resistor
    cir.R("arduino", to_circuit_node(out_node), cir.gnd, arduino_inner_resistance)
    # additional resistor (TODO: this might not be needed)
    cir.R("additional", "voltage", to_circuit_node(in_node), additional_resistance)
    # voltage
...
def to_ltspice_drawing_text(
    ...
    if touch_node is not None:
        ...
        Ws_text += (
            f'WIRE {C_x + capacitor_width // 2} {C_y} {touch_N["x"]} {touch_N["y"]}\n'
        )
...
def _layout_resistors(nodes, links):
    ...
    g = graphviz.Digraph(
        ...
        edge_attr={"arrowhead": "none", "headport": "n", "tailport": "s"},
    )
    g.attr(size="10,10")
    for flink in flinks:
        s, t = flink
...
def _to_ltspice_nodes_wires(links, ltspice_resistors, resistor_height_margin=16):
    ...
    for link, R in zip(links, ltspice_resistors):
        ...
        if not target in Ns:
            Ns[target] = {"x": R["x"] + R["width"] // 2, "y": R["y"] + R["height"]}
        else:
...
if __name__ == "__main__":
    ...
    lcapy_circuit = to_lcapy_circuit(
        nodes, links, resistances=resistances, in_node=in_node, out_node=out_node
    )
    print(lcapy_circuit)
