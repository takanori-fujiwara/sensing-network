...
def default_pipeline(
    ...
    if resistor_link_selection_kwargs["link_weights"] == "length":
        ...
        link_weights[
            (link_weights < length_range[0]) + (link_weights > length_range[1])
        ] = outrange_penalty
    else:
    ...
    la.fit(nodes, links, max_epochs=layout_adjustment_kwargs["layout_learning_epoch"])
    ...
    la.fit(nodes, links, max_epochs=layout_adjustment_kwargs["main_learning_epoch"])
